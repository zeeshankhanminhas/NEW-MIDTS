...
const fieldClass =
  'field_input border-0 border-b border-black/30 bg-transparent px-0 py-3 text-[var(--ink)] outline-none transition placeholder:text-neutral-500 focus:border-black disabled:text-neutral-500';
const labelClass = 'field_group grid gap-2 text-xs font-semibold uppercase text-[var(--ink)]';
export default function EnquiryForm() {
  ...
  return (
    <form className="form_enquiry grid gap-8" onSubmit={handleSubmit}>
      <p className="text_body max-w-xl text-sm leading-6 text-[var(--muted)]">
      ...
      <div className="form_actions grid gap-4 border-t border-black/20 pt-8 md:grid-cols-[auto_1fr] md:items-center">
        <button className="button_primary min-h-12 rounded-md bg-[var(--ink)] px-7 py-3 text-sm font-medium uppercase text-white transition hover:bg-black disabled:cursor-not-allowed disabled:bg-neutral-500 disabled:text-white" type="submit" disabled={isSubmitting}>
          {isSubmitting ? 'Submitting' : 'Start a request'}
        </button>
        <p className="text_body text-sm text-[var(--muted)]">Step 2 follows by email.</p>
      </div>
      {submitted ? (
        <p className="text_success rounded-md border border-black/20 bg-white p-4 text-sm text-[var(--muted)]" role="status">
      ...
      {errorMessage ? (
        <p className="text_error rounded-md border border-black/20 p-4 text-sm text-[var(--ink)]" role="alert">
          {errorMessage}
        </p>
      ) : null}
    </form>
